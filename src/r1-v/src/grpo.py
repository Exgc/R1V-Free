...
temp_dir = '/mnt/private_hk/data/RLHF-V-Dataset-img'
...
def accuracy_reward(completions, solution, image, reward_model, problem, image_path, **kwargs):
    """Reward function that checks if the completion is correct using either symbolic verification or exact string matching."""
    contents = [completion[0]["content"] for completion in completions]
    rewards = []

    current_time = datetime.now().strftime("%d-%H-%M-%S-%f")
    for content, sol, img, pblm, img_path in zip(contents, solution, image, problem, image_path):
        reward = 0.0
        if reward == 0.0:
            try:
...
def main(script_args, training_args, model_args):
    ...
    trainer = trainer_cls(
        ...
        # reward_model='/mnt/private_hk/data/internlm-xcomposer2d5-7b-reward',
